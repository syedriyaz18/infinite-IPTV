document.addEventListener('DOMContentLoaded', () => {
  ...
  async function initApp() {
    ...
    try {
      ...
      if (typeof SplashModule !== 'undefined' && typeof SplashModule.hideSplash === 'function') {
  ...
  // Initialize the app after a short delay to ensure all modules are loaded
  console.log('Setting up app initialization timeout');
  ...
  const splashTimeout = setTimeout(() => {
    ...
    // If we're still on splash screen, show fallback login page
    const pageContainer = document.getElementById('page-container');
    if (pageContainer && pageContainer.querySelector('.splash-container')) {
      console.log('Still on splash screen, showing fallback');
      showFallbackLoginPage();
    }
  }, 5000);
